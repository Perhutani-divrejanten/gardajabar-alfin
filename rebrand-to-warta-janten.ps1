# Rebrand dari Garda Jabar ke Garda Jabar
...
Write-Log -Message "===== REBRAND Garda Jabar DIMULAI ====="
...
foreach ($file in $htmlFiles) {
    try {
        ...
        $replacements = @(
            @{ Old = "Garda Jabar"; New = "Garda Jabar" },
            @{ Old = "GardaJabar"; New = "GardaJabar" },
            @{ Old = "GardaJabar"; New = "GardaJabar" },
            # Social media links
            @{ Old = "https://twitter.com/gardajabar"; New = "https://twitter.com/gardajabar" },
            @{ Old = "https://facebook.com/gardajabar"; New = "https://facebook.com/gardajabar" },
            @{ Old = "https://instagram.com/gardajabar"; New = "https://instagram.com/gardajabar" },
            @{ Old = "https://youtube.com/@gardajabar"; New = "https://youtube.com/@gardajabar" },
            @{ Old = "https://linkedin.com/company/gardajabar"; New = "https://linkedin.com/company/gardajabar" },
            # Email
            @{ Old = "GardaJabar@gmail.com"; New = "GardaJabar@gmail.com" },
            # Logo references
            @{ Old = 'alt="GardaJabar"'; New = 'alt="GardaJabar"' }
        )
...
foreach ($file in $cssFiles) {
    try {
        ...
        # Color replacements
        $colorReplacements = @(
            @{ Old = "#B45309"; New = "#B45309" },          # Primary: kuning -> hijau
            @{ Old = "#B45309"; New = "#B45309" },
            @{ Old = "rgb(255, 204, 0)"; New = "rgb(6, 95, 70)" },
            @{ Old = "#5C2E1A"; New = "#5C2E1A" },          # Secondary: abu -> biru
            @{ Old = "#451A03"; New = "#451A03" },          # Dark: hitam -> darkgreen
            # Additional color mappings if needed
            @{ Old = "#451A03"; New = "#451A03" }
        )
...
foreach ($file in $packageJsonFiles) {
    try {
        ...
        $replacements = @(
            @{ Old = """name"": ""GardaJabar"""; New = """name"": ""GardaJabar""" },
            @{ Old = """name"": ""GardaJabar-article-generator"""; New = """name"": ""GardaJabar-article-generator""" }
        )
...
$searchPatterns = @("Garda Jabar", "GardaJabar", "GardaJabar")
$foundIssues = @()
...
Write-Log -Message ""
Write-Log -Message "Rebrand Garda Jabar selesai ✅"
